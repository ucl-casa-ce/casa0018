# Wake Word example using Edge Impulse

For the lab exercise, use Edge Impulse to train and deploy your own wake word example. Refer to this weeks lecture for more information.

You may find Ch.7 of TinyML book useful for understanding some of the concepts involved in speech detection, but we will not follow the book's example, but instead use Edge Impulse.

Video below shows an example deployed on Arduino Nano.

[![Watch the video](https://img.youtube.com/vi/oKr7YvlxUKQ/maxresdefault.jpg)](https://youtu.be/oKr7YvlxUKQ)
